...
# ---- CdS 照度センサ（A2 ピン）の設定 ----
# ADC(Pin(2, Pin.IN)) : GPIO2 を入力ピンとして ADC に割り当てる
adc2 = ADC(Pin(2, Pin.IN))
# atten(ATTN_11DB) : 減衰率を 11dB に設定 → 0〜3.3V の範囲を読める
adc2.atten(ADC.ATTN_11DB)
# width(WIDTH_12BIT) : 分解能を 12 ビットに設定 → 値の範囲は 0〜4095
adc2.width(ADC.WIDTH_12BIT)

# ---- 土壌水分センサ（A1 ピン）の設定 ----
# 接続・設定方法は CdS と同じ
adc1 = ADC(Pin(1, Pin.IN))
...
while True:
    ...
    print("CdS value:          {}".format(adc2.read()))

    # adc1.read() で土壌水分センサーの ADC 値（0〜4095）を読み取って表示する
    # 土が乾いているほど値が大きく、濡れているほど値が小さくなる
    print("Soil moisture value: {}".format(adc1.read()))
